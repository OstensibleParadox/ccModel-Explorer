...
\node[box, fill=ccL4!35] (L4) at (0,8.1) {\textbf{L4\quad Authority Control}\\[1pt]\scriptsize corporate strategy \,\textbar\, sovereign override --- sets risk budget};
\node[box, fill=ccL2!45] (L2) at (0,5.4) {\textbf{L2\quad Control Blind Spot}\\[1pt]\scriptsize platform controller: system prompt \,\textbar\, guardrails \,\textbar\, moderation \,\textbar\, RAG/context};
\node[box, fill=ccL3, text=white] (L3) at (0,1.9) {\textbf{L3\quad Governance-Object Vacuum}\\[1pt]\scriptsize model internals --- operative state $S_t$ \textit{(hidden)}};
\node[box, fill=ccL1!55] (L1) at (0,-0.8) {\textbf{L1\quad Observable Environment}\\[1pt]\scriptsize controlled process: active user \,\textbar\, passive data-subjects};
...
\draw[ca] ($(L2.south)+(-4.0,0)$) -- node[elbl, pos=0.18]{control action:\\system prompt,\\guardrail, gating} ($(L3.north)+(-4.0,0)$);
