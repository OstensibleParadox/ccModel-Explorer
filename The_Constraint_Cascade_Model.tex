...
Four cases exercise the Cascade's diagnostic function---an environmental disturbance reaching a minor, a known controller defect left unresourced, a sovereign override of corporate safety constraints, and a train-time contamination with no human agent in the loop. Each instantiates a different unsafe control action (\S\ref{sec:uca}), stresses a different friction seam, and shows output-only hazard analysis reaching for the wrong architectural layer.
